...
export const CreateInteractionModal = ({ open, onClose, client_id, editingInteraction }: IProps) => {
    ...
    const onSubmit = (data: { type: string, notes?: string, date: string }) => {
        if (editingInteraction) {
            dispatch(updateInteractionById({
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            ...
            <DialogActions>
                ...
                <Button onClick={handleSubmit(onSubmit)} variant="contained">{editingInteraction ? "Сохранить" : "Добавить"}</Button>
            </DialogActions>
        </Dialog>
    );
};
