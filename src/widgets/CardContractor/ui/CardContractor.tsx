...
} from '@/features/contractors/model/contractorsApi.ts';
...
export const CardContractor = ({ contractorId }: Props) => {
    ...
    if (isError || !contractorData)
        return <Typography sx={{ mt: 4, textAlign: 'center' }}>Ошибка загрузки данных</Typography>;

    return (
        <Card sx={{ p: 2 }}>
            <CardContent sx={{ display: 'flex', gap: 3 }}>
                <Avatar sx={{ width: 80, height: 80, fontSize: 32 }}>
                    {watchedValues.name?.[0] || 'П'}
                </Avatar>
                ...
                <Box sx={{ flexGrow: 1 }}>
                    {isEditing ? (
                        ...
                        <>
                            ...
                            <Typography color="text.secondary">
                                👤 Контактное лицо: {watchedValues.contact_person}
                            </Typography>
                            <Typography color="text.secondary">
                                🛠️ Специализация: {watchedValues.specialization}
                            </Typography>
                            <Typography color="text.secondary">
                                📝 Дополнительная информация: {watchedValues.info}
                            </Typography>
                            <Typography color="text.secondary">
                                📍 Адрес: {watchedValues.address}
                            </Typography>
                        </>
                    )}
                    ...
                    <Typography color="text.secondary" sx={{ mt: 2 }}>
                        ⭐ Средняя оценка:{' '}
                        {watchedValues.average_rating
                            ? Number(watchedValues.average_rating).toFixed(1)
                            : '—'}
                    </Typography>
                </Box>
