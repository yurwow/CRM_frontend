...
export const ClientInf = () => {
    ...
    const handleSave = () => {
        if (currentClient) {
            dispatch(updateClient({ id: Number(id), clientData: clientData as IAddClient }));
            setIsEditing(false);
        }
    };
        ...
        <Box sx={{ display: "flex", justifyContent: "center", mt: 4 }}>
            ...
            >
                <CardContent>
                    ...
                    <Stack spacing={2}>
                        {[
                            { icon: <ContactMailIcon />, label: "Контактное лицо", name: "contact_person", value: clientData?.contact_person || "", color: "primary.main" },
                            { icon: <PhoneIcon />, label: "Телефон", name: "phone", value: clientData?.phone || "", color: "green" },
                            { icon: <EmailIcon />, label: "Email", name: "email", value: clientData?.email || "", color: "red" },
                            { icon: <HomeIcon />, label: "Адрес", name: "address", value: clientData?.address || "", color: "orange" },
                            { icon: <BusinessIcon />, label: "Индустрия", name: "industry", value: clientData?.industry || "", color: "purple" }
                            ...
                            >
                                <Box sx={{ color, display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36 }}>
                                    {icon}
                                </Box>
                                {isEditing ? (
                                    <TextField
